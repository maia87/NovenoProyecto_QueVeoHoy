var conexion = require('../lib/conexionbd'); //se importa la conexión a la base de datos

function obtenerListado(req, res) {
  //se agregan los filtros y los valores correspondientes que llegan en la request
  //si en la request llega como parametro la cantidad de resultados a devolver, se setea el limite que corresponde,
  //sino se define que van a ser 52 resultados
  ...
  //se inicializan los filtros y se van agregando los parametros por los cuales se va a filtrar si los mismo llegaron como parametro en la request
  var filtros = "";
  if (req.query.titulo) {
  ...
  //se ejecuta la consulta para obtener las peliculas que correspondan segun los filtros y parametros recibidos
  conexion.query(sql,
    function(error, resultado_peliculas, fields) {
      //luego de finalizar la ejecución de la primer consulta, se realiza una segunda para obtener el total de películas
      ...
      conexion.query(sql2,
        function(error, total, fields) {
          ...
          //se agrega a la respuesta el total de películas
          var respuesta = {
            'total': total[0].total
          };
          //se agrega a la respuesta los datos de las películas
...
function obtenerPelicula(req, res) {
  ...
  conexion.query(sql,
    function(error, pelicula, fields) {
      //se realiza una segunda para obtener los datos de los actores
      ...
      function(error, actores, fields) {
        ...
        //si no se encontro ninguna película con el ID correspondiente, se envía un mensaje de error
